def call_paragraph():
    # Here we are trying to accept the  paragraph from user as an input, Incase if it is   not provided, Input prompt re-appears untill a valid string is provided.
    while True:
        try:
            ...
            return paragraph
             ...
             #For every skip of input (paragraph) will logging an error  message 
            print(f"A paragraph Input is skipped: {ve}")


def call_page_width():
    # Here we are trying to accept the  pagewidth from user as an input, Incase if it is not provided the input prompt re-appears untill a valid iinterger is provided.
    while True:
        try:
          ...
          #For every skip of input (page-width) will logging an error  message   
           # print(f"Error: {ve}")
            print(f"A page width Input is skipped: {ve}")
...
def justify_paragraph(paragraph, page_width):
    ...
    for word in words:
        ...
        if current_line_length + len(current_line) + len(word) <= page_width:
            ...
            # if current line exceeds the given page width, the adding a new line
